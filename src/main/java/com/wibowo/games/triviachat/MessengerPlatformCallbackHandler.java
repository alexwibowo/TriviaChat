...
public class MessengerPlatformCallbackHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(MessengerPlatformCallbackHandler.class.getName());

    private static final ConcurrentHashMap<String, ChatStateMachine> chatStateMachines = new ConcurrentHashMap<>();
    ...
    public MessengerPlatformCallbackHandler(final Messenger messenger,
        ...
        try {
            ...
            topicRepository.addTopic(googleSheetReader.retrieveTopic("RESPIRATORY", "Respiratory!A2:I"));
    ...
    private void handleTextMessageEvent(final ChatStateMachine chatStateMachine,
        ...
        chatStateMachine.process(messageText);
    ...
    private void sendButtonMessage(final String recipientId,
                                   final List<String> options) {
        try {
            final List<Button> buttons = options.stream().map((Function<String, Button>) s ->
                    PostbackButton.create(s, s)).collect(Collectors.toList()
            );
    ...
    private void sendQuickReply(final String recipientId,
                                final List<String> options) {
        try {
            final List<QuickReply> quickReplies = options.stream().map((Function<String, TextQuickReply>) s ->
                    TextQuickReply.create(s, s)).collect(Collectors.toList()
            );

            final TextMessage message = TextMessage.create("Your answer:", of(quickReplies), empty());
    ...
    private void handleQuickReplyMessageEvent(final ChatStateMachine chatStateMachine,
                                              final QuickReplyMessageEvent event) {
        logger.debug("Handling QuickReplyMessageEvent");
        ...
        chatStateMachine.process(payload);
        for (final String machineResponse : chatStateMachine.machineResponses()) {
            sendTextMessage(senderId, machineResponse);
        }
        final List<String> answers = chatStateMachine.availableUserOptions().stream().map(Object::toString).collect(Collectors.toList());
    ...
    private void handlePostbackEvent(final ChatStateMachine chatStateMachine,
                                     final PostbackEvent event) {
        ...
        sendButtonMessage(senderId, answers);
    }
